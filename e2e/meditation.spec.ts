 ...
 * Uses zustand persist storage key 'beesvat-auth'.
 */
...
test.describe('Meditation list page', () => {
  test('should show meditation list when visiting without authentication', async ({ page }) => {
...
test.describe('New meditation page', () => {
  test('should show new meditation form when visiting without authentication', async ({ page }) => {
  ...
  test('should display meditation content when authenticated', async ({ page }) => {
    ...
    await page.route('**/api/v1/meditations/meditation-1', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ data: mockMeditation }),
      });
    });
