...
import { getVerbReferences } from '@/server/services/verb.service';
import { verbReferencesQuerySchema } from '@/server/schemas/verb.schema';

// FEAT-2: Get verb references endpoint
export async function GET(request: NextRequest, { params }: { params: Promise<{ word: string }> }) {
  try {
    ...
    const parsed = verbReferencesQuerySchema.safeParse({
    ...
    const result = await getVerbReferences(word, parsed.data);
