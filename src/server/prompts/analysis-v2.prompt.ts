...
**JSON의 mainVerbs 배열에는 반드시 최종 선정된 정동사 3~5개만 포함하세요.**
**절대 5개를 초과하지 마세요. 중간 과정(전체 동사 목록, 필터링 결과)은 JSON에 포함하지 마세요.**

### 내부 선정 과정 (JSON에 포함하지 말 것):
1. 본문의 모든 동사를 확인
2. 분사(Participle)·부정사(Infinitive)를 제외하고 정동사(finite verb)만 남김
3. 남은 정동사 중 신학적으로 핵심적인 3~5개만 선정
...
### 제외 대상:
- λέγω(말하다), אָמַר(말하다) 같은 일반적 발화 동사
- 분사(Participle), 부정사(Infinitive) 등 부동사(non-finite)
